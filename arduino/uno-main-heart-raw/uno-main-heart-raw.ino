...
const unsigned long HEART_DISPLAY_CYCLE_MS = 30000;
...
void showHeartRawDisplay(int heartRawA, bool fingerDetectedA, int heartRawB, bool fingerDetectedB) {
  char lineA[28];
  char lineB[28];
  ...
  do {
    ...
    u8g2.drawStr(0, 12, "HEART RAW MONITOR");
    snprintf(lineA, sizeof(lineA), "RED   %4d %s", heartRawA, fingerDetectedA ? "OK" : "--");
    snprintf(lineB, sizeof(lineB), "GREEN %4d %s", heartRawB, fingerDetectedB ? "OK" : "--");
    u8g2.drawStr(0, 34, lineA);
    u8g2.drawStr(0, 54, lineB);
  } while (u8g2.nextPage());
}
...
void refreshDisplay(int heartRawA, bool fingerDetectedA, int heartRawB, bool fingerDetectedB) {
  ...
  if (shouldShowHeartWindow(now)) {
    showHeartRawDisplay(heartRawA, fingerDetectedA, heartRawB, fingerDetectedB);
    return;
  }
  showReadyDisplay();
}
